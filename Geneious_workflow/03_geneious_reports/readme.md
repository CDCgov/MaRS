...
* Once the code finishes running, different outputs will be generated (* denotes a wild card that will be a two letter code specific to the country being analyzed):
     ...
     * *_Bar_plot_Combined3: Bar graph shows the wild type, major and minor allele frequencies of all the reportable SNPs described in Voinew3.csv file. Allele frequencies are indicated on the x axis, and the variants of interest and total number of samples are listed along the y-axis.          The color coding indicates the type of mutation found in the samples; blue is for wild type, green for minor allele mutation and red for major allele mutation.
